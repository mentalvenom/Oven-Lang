...
def __new_num(env, _, args):
  return make_empty_obj(oven_num)
...
def __init_num(env, _, args):
  ...
  if len(args) == 0:
    ...
    raise OvenError("Too many arguments passed to num()")

def __add_num(env, _, args):
  assert len(args) == 2
  if args[1].obj_type is oven_num:
...
def to_int(num):
  if is_int(num.val):
    ...
    raise OvenError(f"Cannot convert {num.val} to integer")
...
oven_num = Object(oven_type, **num_attrs)


def __new_bool(env, _, args):
  return make_empty_obj(oven_bool)
...
def __init_bool(env, _, args):
  ...
  if not args:
    ...
    raise OvenError("Too many arguments passed to bool")
...
oven_bool = Object(oven_type, **bool_attrs)

def __new_string(env, _, args):
  return make_empty_obj(oven_string)
...
def __add_string(env, _, args):
  assert len(args) == 2
  if args[1].obj_type is oven_string:
...
def __num_string(env, _, args):
  ...
  try:
    ...
    try:
      ...
      raise OvenError("Can't cast {val} to number")
...
oven_string = Object(oven_type, **string_attrs)


def __new_list(env, _, args):
  return make_empty_obj(oven_list)
...
def __add_list(env, _, args):
  assert len(args) == 2
  if args[1].obj_type is oven_list:
...
def __in_list(env, _, args):
  ...
  for elem in args[0].vals:
    if make_bool(eq(args[1], elem)).val:
      return bool_from_py_bool(True)
  return bool_from_py_bool(False)
...
def pop_list(env, _, args):
  if args[1].vals:
    ...
    raise OvenError("Pop from empty list")
...
oven_list = Object(oven_type, **list_attrs)


def __new_dict(env, _, args):
  return make_empty_obj(oven_func)

dict_attrs = {

}

oven_dict = Object(oven_type, **dict_attrs)
...
oven_func = Object(oven_type, **func_attrs)
