...
for type_name in ERR_TYPES:
  globals()[type_name] = type(type_name, (OvenError,), {})
...
class Object(): # the object type class
  ...
  def special_meth(name, err_str, err_type, right=None):
    def meth(self, env, inst, args):
      if name and name in self.attrs:
        ...
        if res is not oven_notimpl:
          return res
      
      if right and right in args[0].obj_type.attrs:
        new_recv = args[0]
        new_args = [inst] + args[1:]
        func = new_recv.obj_type.attrs[name]
        res = func.obj_type.call(env, func,
                                 new_args, sub_success)
  ...
  contains = special_meth(None, "Can't test {} for membership",
                          CompareError, "__in")
  rcontains = special_meth("__in", "Can't test {} for membership",
                           CompareError)
...
oven_type = Type(None) 
oven_type.obj_type = oven_type
oven_object = Object(Type)

oven_notimpl_type = Object(oven_type)
oven_notimpl = Object(oven_notimpl_type)
oven_none_type = Object(oven_type)
oven_none = Object(oven_none_type)

class BuiltinFuncType(Object):
  def __init__(self, *args, **kwargs):
    def __new():
      raise OvenError("Cannot make BuiltinFunc instance")
...
class BuiltinFunc(Object):
  def __init__(self, func, *args, **kwargs):
    self.func = func
    super().__init__(*args, oven_builtin, **kwargs)
  
  def __repr__(self):
    return f"builtin-{self.func.__qualname__}"

oven_builtin = None # bootstrap the type for __new and init
oven_builtin = BuiltinFuncType(oven_type)
oven_builtin.attrs["__new"].obj_type = oven_builtin
oven_builtin.attrs["__init"].obj_type = oven_builtin
