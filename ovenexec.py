import ovenast as ast
import ovenobjects as objs
import oventokenizer as tokenizer
import ovenparser as parser
...
def eval_expr(env, expr):
  if isinstance(expr, ast.UnaryExpr):
    ...
    if expr.name in val.attrs:
      ...
      raise objs.OvenError(f"Can't find attribute {expr.name}")
    ...
    if expr.name in expr_val.attrs:
      ...
      raise obs.OvenError(f"Can't find attribute {expr.name}")
